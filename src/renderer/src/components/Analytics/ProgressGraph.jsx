...
const ProgressGraph = ({ data = [], height = 240 }) => {
  ...
  const accPathData = points.reduce(
    ...
    <div className="progress-graph-wrapper" ref={containerRef}>
      <div className="graph-header-meta">
        <div className="meta-item">
          ...
          <span className="value">
            {Math.max(...values)} <small>WPM</small>
          </span>
        </div>
        <div className="meta-item">
          <span className="label">AVG SPEED</span>
          <span className="value">
            {Math.round(values.reduce((a, b) => a + b, 0) / values.length)} <small>WPM</small>
          </span>
        </div>
        <div className="meta-item">
      ...
      <div className="svg-container">
        ...
        >
          ...
          {points
            .filter((_, idx) => idx % Math.ceil(points.length / 5) === 0)
            .map((p, i) => (
              <line
                key={i}
                x1={p.x}
