...
const TelemetryGraph = ({ data = [], width = 500, height = 120 }) => {
  ...
  const wpmPath = points.reduce((acc, p, i) => 
    i === 0 ? `M ${p.x},${p.yWpm}` : `${acc} L ${p.x},${p.yWpm}`, 
  "");

  const rawPath = points.reduce((acc, p, i) => 
    i === 0 ? `M ${p.x},${p.yRaw}` : `${acc} L ${p.x},${p.yRaw}`, 
  "");
      ...
      <div className="telemetry-svg-wrapper">
        ...
        >
          ...
          {[0.25, 0.5, 0.75].map(v => (
            <line 
                key={v}
                x1="0" y1={height * v} x2={width} y2={height * v} 
          ...
          {points.map((p, i) => (
            <rect 
              key={i}
              x={p.x - 5} y={0} width={10} height={height}
              fill="transparent"
          ...
          {hoveredIndex !== null && (
             <motion.div 
               ...
               animate={{ 
                  opacity: 1, 
                  y: 0,
                  left: points[hoveredIndex].x > width - 120 ? points[hoveredIndex].x - 120 : points[hoveredIndex].x + 15
               }}
               exit={{ opacity: 0 }}
               style={{ 
                  position: 'absolute',
